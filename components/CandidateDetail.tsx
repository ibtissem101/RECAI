              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                ...
                <div className="md:col-span-3 flex flex-col justify-center space-y-5">
                  ...
                  <p className="text-xs text-slate-400 italic pt-2 border-t border-slate-100 mt-2">
                    * Scores are calculated using semantic analysis of the
                    resume against the job description, verified by external web
                    data sources.
                  </p>
                </div>
              </div>
            ...
            <div className="bg-white rounded-xl border border-[#BDDEF3] shadow-sm p-6">
              ...
              <div className="relative pl-2 space-y-8">
                ...
                {candidate.previousCompanies.map((job, idx) => (
                  ...
                  >
                    <div className="w-11 h-11 rounded-full bg-white border border-slate-200 flex items-center justify-center z-10 shadow-sm group-hover:border-[#3f5ecc] transition-colors">
                      <Briefcase className="w-5 h-5 text-slate-400 group-hover:text-[#3f5ecc]" />
                    </div>
                    <div className="flex-1 bg-slate-50 p-4 rounded-lg border border-slate-100 hover:border-[#BDDEF3] transition-colors">
                      ...
                      <p className="text-sm font-medium text-[#3f5ecc] mb-2">
                        {job.name}
                      </p>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {["System Design", "Leadership", "Scaling"].map(
                          (tag) => (
                            <span
                              key={tag}
                              className="text-xs text-slate-500 bg-white px-2 py-0.5 rounded border border-slate-200"
                            >
                              {tag}
                            </span>
                          )
        ...
        return (
          <div className="bg-white rounded-xl border border-[#BDDEF3] shadow-sm p-8 min-h-[600px]">
            <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-100">
              ...
              <button className="text-[#3f5ecc] text-sm font-medium flex items-center gap-2 hover:underline">
                <Download className="w-4 h-4" /> Download Original PDF
              </button>
            </div>
            <div className="prose prose-slate max-w-none text-slate-600 font-normal">
              <p className="text-xl font-bold text-slate-800 mb-0">
                {candidate.name}
              </p>
              ...
              <h4 className="text-slate-800 mt-6 mb-2 font-bold uppercase text-sm">
                Summary
              </h4>
              <p className="mb-4">
                Senior Backend Engineer with {candidate.experienceYears} years
                of experience building scalable distributed systems. Expert in
                Python, Django, and cloud infrastructure.
              </p>
              ...
              {candidate.previousCompanies.map((job, i) => (
                <div key={i} className="mb-4">
                  ...
                  <ul className="list-disc pl-5 space-y-1 text-sm">
                    <li>
                      Led development of core payment processing infrastructure
                      handling $1B+ volume.
                    </li>
                    <li>
                      Architected microservices migration using{" "}
                      {candidate.skills[0] || "Python"} and{" "}
                      {candidate.skills[1] || "AWS"}.
                    </li>
                    <li>
                      Mentored junior engineers and established code quality
                      standards.
                    </li>
                  </ul>
                </div>
      ...
      case "interview":
        return (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-[#BDDEF3] shadow-sm p-6">
              <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
                <CheckCircle2 className="w-5 h-5 text-slate-500" /> Scorecard
                Template
              </h3>
                  ...
                  >
                    ...
                    <div className="flex gap-1">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <button
                          key={star}
                          className="w-8 h-8 rounded-full border border-slate-200 hover:border-[#3f5ecc] hover:bg-[#eef2ff] text-slate-400 hover:text-[#3f5ecc] flex items-center justify-center transition-colors"
                        >
                          {star}
                        </button>
                      ))}
  ...
  return (
    <div className="flex flex-col h-full bg-[#F5F9FC] overflow-hidden">
      ...
      <div className="bg-white border-b border-[#BDDEF3] px-6 py-4 flex items-center gap-4 sticky top-0 z-20 shadow-sm flex-shrink-0">
        <button
          onClick={onBack}
          className="p-2 -ml-2 text-slate-400 hover:text-[#3f5ecc] hover:bg-[#f5f7ff] rounded-full transition-colors"
        >
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => navigate(`/candidates/${candidate.id}/review`)}
            className="px-4 py-2 bg-white border border-[#3f5ecc] text-[#3f5ecc] rounded-lg text-sm font-medium hover:bg-[#eef2ff] transition-colors flex items-center gap-2"
          >
            <Shield className="w-4 h-4" /> HR Review
          </button>
          <div className="h-6 w-px bg-slate-200 mx-1"></div>
          <button className="px-4 py-2 bg-[#3f5ecc] text-white rounded-lg text-sm font-medium hover:bg-[#3552b8] shadow-sm shadow-[#3f5ecc]/20 flex items-center gap-2">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-7xl mx-auto p-6 space-y-6">
          ...
          <div className="bg-white rounded-xl border border-[#BDDEF3] shadow-sm p-6 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-64 h-64 bg-[#eef2ff]/20 rounded-bl-[100px] -mr-16 -mt-16 pointer-events-none"></div>

            <div className="flex flex-col md:flex-row gap-6 relative z-10">
              {/* Avatar / Initials */}
              <div className="w-20 h-20 rounded-2xl bg-[#3f5ecc] text-white flex items-center justify-center text-2xl font-bold shadow-lg shadow-[#3f5ecc]/20 flex-shrink-0">
                {candidate.name
                  .split(" ")
                  .map((n) => n[0])
                  .join("")}
              ...
              <div className="flex-1">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-4 text-sm text-slate-500 mt-1">
                      <span className="flex items-center gap-1.5">
                        <MapPin className="w-4 h-4 text-[#3f5ecc]" />{" "}
                        {candidate.webVerification.salaryBenchmark.location}
                      </span>
                      <span className="flex items-center gap-1.5">
                        <Briefcase className="w-4 h-4 text-[#3f5ecc]" />{" "}
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  {candidate.webVerification.linkedin?.verified && (
                    <a
                      href="#"
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-[#0077b5]/10 text-[#0077b5] rounded-md text-xs font-medium hover:bg-[#0077b5]/20 transition-colors"
                    >
                  ...
                  {candidate.webVerification.github?.verified && (
                    <a
                      href="#"
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 text-slate-700 rounded-md text-xs font-medium hover:bg-slate-200 transition-colors"
                    >
              ...
              <div className="flex gap-6 border-l border-slate-100 pl-6 md:pl-0 md:border-l-0">
                <div className="text-center">
                  <div className="text-3xl font-bold text-[#3f5ecc]">
                    {candidate.score}
                  </div>
                ...
                <div className="text-center">
                  <div className="text-3xl font-bold text-slate-700">
                    {candidate.webVerification.github?.contributions || "N/A"}
                  </div>
                  <div className="text-xs font-medium text-slate-500 uppercase tracking-wide mt-1">
                    Contribs
                  </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-xl border border-[#BDDEF3] shadow-sm p-6">
                <h3 className="text-sm font-bold text-slate-800 mb-4 flex items-center gap-2 uppercase tracking-wide">
                  <Code className="w-4 h-4 text-[#3f5ecc]" /> Skills Matrix
                </h3>
                <div className="space-y-4">
                  {candidate.skills.map((skill, i) => (
                    <div key={i}>
                      <div className="flex justify-between text-xs mb-1">
                        ...
                        <span className="text-[#3f5ecc] font-bold">
                          Verified
                        </span>
                      </div>
                      <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                        <div
                          className="bg-[#3f5ecc] h-full rounded-full"
                          style={{ width: `${90 - i * 10}%` }}
                        ></div>
                      </div>
                    </div>
              ...
              <div className="bg-white rounded-xl border border-[#BDDEF3] shadow-sm p-6">
                <h3 className="text-sm font-bold text-slate-800 mb-4 flex items-center gap-2 uppercase tracking-wide">
                  <DollarSign className="w-4 h-4 text-[#3f5ecc]" /> Compensation
                </h3>

                <div className="text-center py-4 bg-[#f5f7ff] rounded-lg border border-[#BDDEF3] border-dashed mb-4">
                  ...
                  <p className="text-xs text-[#3f5ecc] font-medium mt-1">
                    Within Budget
                  </p>
                </div>
              ...
              <div className="bg-white rounded-xl border border-[#BDDEF3] shadow-sm p-6">
                <h3 className="text-sm font-bold text-slate-800 mb-4 flex items-center gap-2 uppercase tracking-wide">
                  <Clock className="w-4 h-4 text-[#3f5ecc]" /> Next Steps
                </h3>
                <div className="space-y-3">
                  <button className="w-full py-2 bg-[#3f5ecc] hover:bg-[#3552b8] text-white text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-2">
                    <Calendar className="w-4 h-4" /> Schedule Interview
                  </button>
                  <button className="w-full py-2 bg-white border border-[#BDDEF3] hover:bg-[#f5f7ff] text-slate-700 text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-2">
