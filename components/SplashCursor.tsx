...
export default function SplashCursor({
  ...
  // keep click multiplier modest so blue stays subtle
  color.r *= 2;
  color.g *= 2;
  color.b *= 2;
  const dx = 2 * (Math.random() - 0.5);
  const dy = 6 * (Math.random() - 0.5);
  splat(pointer.texcoordX, pointer.texcoordY, dx, dy, color);
   ...
   // brand blue base: #3F5ECC -> (63,94,204)
function generateColor(): ColorRGB {
  const base = { r: 63 / 255, g: 94 / 255, b: 204 / 255 };

  // small variation so the effect looks organic but stays blue
  const variance = 0.18; // 0 = single flat blue, larger = more variety (0.1-0.25 good)
  const factor = 0.2 + Math.random() * 0.3; // controls brightness jitter

  return {
    r: base.r * factor * (1 - variance * Math.random()),
    g: base.g * factor * (1 - variance * Math.random()),
    b: base.b * factor * (1 - variance * Math.random())
  };
}
